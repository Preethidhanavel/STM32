...
DMA_HandleTypeDef hdma_spi1_rx;  // DMA handle for SPI RX
DMA_HandleTypeDef hdma_spi1_tx;  // DMA handle for SPI TX
UART_HandleTypeDef huart2;       // UART2 handle for debugging/output
/* USER CODE BEGIN PV */
...
int main(void)
{
  ...
  MX_SPI1_Init();
  /* USER CODE BEGIN 2 */
  AL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_RESET);  // Pull CS LOW to select slave
  ...
  {
    /* USER CODE END WHILE */
	   // Check if previous SPI transfer is complete
        if (spi_ready)
        {
