...
uint8_t tx_buffer[] = "Hello SPI";  // Data to transmit over SPI
...
UART_HandleTypeDef huart2; // UART2 handle for debug output
/* USER CODE BEGIN PV */
...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
/* Callback function called when SPI transmit/receive completes */
void HAL_SPI_TxRxCpltCallback(SPI_HandleTypeDef *hspi)
{
    if (hspi->Instance == SPI1) {
        HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_SET); // Set CS HIGH (deselect slave)

        // Send received SPI data via UART for debugging
    ...
    spi_done = 1; // Set flag to indicate SPI is done and ready for next transfer
}
/* USER CODE END 0 */
...
int main(void)
{
  ...
  MX_SPI1_Init();
  /* USER CODE BEGIN 2 */
   HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_RESET); // Set CS LOW to select slave

    // Start SPI transfer using interrupt mode (non-blocking)
  ...
  {
    /* USER CODE END WHILE */
	 // If SPI transfer is done, restart transfer
        if (spi_done)
        {
            memset(rx_buffer, 0, sizeof(rx_buffer));       // Clear RX buffer
            spi_done = 0;                                 // Reset flag
            HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_RESET); // Select slave
            HAL_SPI_TransmitReceive_IT(&hspi1, tx_buffer, rx_buffer, sizeof(tx_buffer)); // Start next transfer
            HAL_Delay(1000); // Optional delay between transfers
        }
    /* USER CODE BEGIN 3 */
  }
  /* USER CODE END 3 */
